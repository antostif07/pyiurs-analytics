...
export function CreateUserModal({ isOpen, onClose, onUserCreated, shops, companies }: CreateUserModalProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.password && !formData.send_invitation) {
      newErrors.password = 'Le mot de passe est requis si vous n\'envoyez pas d\'invitation'
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Mot de passe et invitation */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Mot de passe {!formData.send_invitation && '*'}
              </label>
              <input
                type="password"
                value={formData.password}
                onChange={(e) => setFormData(prev => ({ ...prev, password: e.target.value }))}
                disabled={formData.send_invitation}
                className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  errors.password 
                    ? 'border-red-500 dark:border-red-400' 
                    : 'border-gray-300 dark:border-gray-600 dark:bg-slate-700'
                } ${formData.send_invitation ? 'opacity-50 cursor-not-allowed' : ''}`}
                placeholder="••••••••"
              />
              {errors.password && (
