...
import { ArrowLeft, Building, Calendar, Download, Eye, FileText, Search } from "lucide-react"
import Link from "next/link"

export function Historique({clotures}: {clotures: ClotureDataView[]}) {
    ...
    const handleDownloadPDF = async (cloture: ClotureDataView) => {
        try {
            const pdfData = {
            closure: cloture,
            mainCash: cloture.cash_closure_main_cash || [],
            secondaryCash: cloture.cash_closure_secondary_cash || [],
            denominations: (cloture.denominations || []).map(d => ({
                id: d.id || '',
                cash_closure_id: d.cash_closure_id,
                currency: d.currency,
            ...
            shopInfo: {
                name: cloture.shop_name || `Boutique #${cloture.shop_id}`
            }
            }

            const blob = await pdfService.generateCloturePDF(pdfData)
            const url = URL.createObjectURL(blob)
            const link = document.createElement('a')
            link.href = url
            link.download = `cloture-${cloture.id}-${format(new Date(cloture.closing_date), 'dd-MM-yyyy')}.pdf`
            link.click()
                            ...
                            <TableRow key={cloture.id}>
                                <TableCell className="font-medium">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {formatPeriod(cloture.opening_date, cloture.closing_date)}
                                    </div>
                                </TableCell>
                                <TableCell>
                                <div className="flex items-center gap-2">
                                ...
                                <TableCell className="text-center flex gap-2">
                                    ...
                                    <Button 
                                        variant="outline" 
                                        size="sm"
                                        onClick={() => handleDownloadPDF(cloture)}
                                    >
