...
export function AuditScannerClient({ audit, initialItems }: AuditScannerClientProps) {
    ...
    const handleScanSubmit = async (e: React.FormEvent) => {
        ...
        if (result.success) {
            ...
            toast.success(`Scanné avec succès (1/1) : ${cleanCode}`);
            router.refresh();
    ...
    // IMPORTATION EXCEL AVEC SELECTION DE COLONNE
    const handleExcelImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Détection basique de la colonne code-barres
        lines.forEach((line) => {
            const cols = line.split(/[,;\t]/);
            const code = cols[0]?.trim();
            if (code && !code.toLowerCase().includes("code")) {
                barcodes.push(code);
            }
        });
        ...
        toast.info(`Importation de ${barcodes.length} codes-barres...`);
        const res = await importExcelBarcodesAction(audit.id, barcodes);

        if (res.success) {
            toast.success(`${res.successCount} articles scannés avec succès depuis le fichier !`);
            router.refresh();
        } else {
    ...
    // EXPORTATION EXCEL (.CSV/.XLSX)
    const handleExportExcel = () => {
        const headers = ["Code-barres", "Produit", "Stock Théorique Odoo", "Compté Physique", "Écart Qté", "Coût Unitaire ($)", "Démarque ($)"];
        const rows = initialItems.map((i) => [
            i.internal_barcode,
    ...
    // EXPORTATION PDF (Impression officielle)
    ...
    const scannedItemsCount = initialItems.filter((i) => (i.counted_qty || 0) === 1).length;
    ...
    }, [initialItems, filterSearch, statusFilter]);
    ...
    return (
        <div className="space-y-6 pb-12 animate-in fade-in duration-300 print:p-0">
                    ...
                    {!isReadOnly && (
                        <>
                            ...
                            <label className="h-9 px-3 rounded-xl text-xs font-semibold bg-muted/30 hover:bg-muted/60 border border-border text-foreground flex items-center gap-1.5 cursor-pointer transition-colors">
                                ...
                                <input type="file" accept=".csv,.txt" onChange={handleExcelImport} className="hidden" />
                            </label>

                            {/* Validation Final */}
                            <AlertDialog>
                                <AlertDialogTrigger asChild>
                                    <Button disabled={isValidating || totalItemsCount === 0} className="h-9 px-4 rounded-xl font-bold text-xs bg-emerald-600 hover:bg-emerald-700 text-white gap-2 cursor-pointer shadow-xs">
                                        <CheckCircle2 className="w-4 h-4" /> Clôturer l'Audit
                                    </Button>
                                </AlertDialogTrigger>
                                <AlertDialogContent className="bg-card text-card-foreground border-border rounded-2xl max-w-md">
                                    <AlertDialogHeader>
                                        <AlertDialogTitle className="text-base font-bold">Clôturer l'Audit {audit.reference} ?</AlertDialogTitle>
                                    ...
                                    <AlertDialogFooter className="gap-2 pt-2">
                                        ...
                                        <AlertDialogAction onClick={() => validateAuditSessionAction(audit.id).then(() => router.refresh())} className="h-9 rounded-xl text-xs font-semibold bg-emerald-600 text-white cursor-pointer">
                                            Valider et Clôturer
                                        </AlertDialogAction>
                                    </AlertDialogFooter>
                                </AlertDialogContent>
                ...
                <div className="bg-card border-2 border-primary/30 rounded-2xl p-4 shadow-sm space-y-2 print:hidden">
                    <div className="flex items-center justify-between">
                        <label className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5">
                            <ScanLine className="w-4 h-4 text-primary animate-pulse" /> Scan Pistolet Laser (Unitaire Strict 0 ou 1)
                        </label>
            ...
            {/* 3. BARRE DE PROGRESSION & COMPTEURS EN TEMPS RÉEL */}
