// customer-segmentation-matrix-table.tsx
"use client";
...
// Données exactes de la capture d'écran
const DEFAULT_CUSTOMER_DATA: CustomerSegmentRow[] = [
    {
        segment: "Platinum",
        totalCustomers: 80,
        grossAdds: 6,
...
// Couleurs badges subtiles pour les tiers de fidélité luxe
function getSegmentBadge(segment: string) {
    switch (segment) {
        case "Platinum":
            return "text-slate-800 dark:text-slate-200 font-black";
        case "Gold":
            return "text-amber-600 dark:text-amber-400 font-bold";
        case "Silver":
            return "text-slate-500 dark:text-slate-400 font-semibold";
        default:
            return "text-slate-600 dark:text-slate-400 font-normal";
    }
}

export default function CustomerSegmentationMatrixTable({
        ...
        () => [
            {
                ...
                cell: (info) => {
                    ...
                    return (
                        <span className={cn("text-[11px]", getSegmentBadge(val))}>
                            {val}
                        </span>
                    );
                },
    ...
    return (
        <section className="space-y-2">
            {/* Titre avec le bandeau doré conforme à l'image */}
            ...
            {/* Conteneur compact Shadcn UI Table */}
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <Table className="w-full text-[11px]">
                        {/* Header bleu nuit dense */}
                                    ...
                                    {headerGroup.headers.map((header, idx) => (
                                        <TableHead
                                            ...
                                            className={cn(
                                                ...
                                                idx === 0
                                                    ? "text-left sticky left-0 z-20 bg-[#101c30] min-w-[140px]"
