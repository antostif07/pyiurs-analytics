...
import { ClotureVenteCloseProps, CaissePrincipaleRow, CaisseSecondaireRow } from "@/lib/types"
...
import { MainCashTable } from "./main-cash-table"
import { format } from "date-fns"
import { BilleterieCard } from "./billeterie-card"
...
export default function ClotureVenteClose({
  ...
  const calculations = useMemo(() => {
    const usd = denominations.filter(d => d.currency === 'USD').reduce((sum, d) => sum + (d.value * d.quantity), 0)
    const cdf = denominations.filter(d => d.currency === 'CDF').reduce((sum, d) => sum + (d.value * d.quantity), 0)
    const calculated = usd + (cdf / initialData.exchangeRate)
    return { physicalCashUSD: usd, physicalCashCDF: cdf, calculatedCash: calculated, difference: calculated - initialData.expectedCash }
  }, [denominations, initialData.exchangeRate, initialData.expectedCash])

  const expensesByCash = useMemo(() => {
    // Note: Logique conservée exactement comme dans l'original
    return initialData.expenses.reduce((acc, expense) => {
      const isEpargne = expense.journal_id && typeof expense.journal_id === 'object' 
          ...
          : false;
      const filteredExps = expense.expenses
      ...
      if (isEpargne) {
          acc.caisseEpargne.push(...filteredExps);
          acc.totalEpargne += expense.total_amount || 0;
      } else {
          acc.caissePrincipale.push(...filteredExps);
          acc.totalPrincipal += expense.total_amount || 0;
      }
      return acc;
    }, { caissePrincipale: [], caisseEpargne:[], totalPrincipal: 0, totalEpargne: 0 } as ExpenseByCash);
  }, [initialData]);
  ...
  useEffect(() => {
    setCaissePrincipaleData([
        {
          modePaiement: "Espèces", paymentMethod: "especes", paymentMethodId: 1, soldeOuverture: 100,
          ventesJour: initialData.cashSalesTotal, sortiesJour: expensesByCash.totalPrincipal,
          clotureTheorique: 100 + initialData.cashSalesTotal - expensesByCash.totalPrincipal,
          cashPhysique: calculations.calculatedCash, managerConfirmed: false, financierConfirmed: false
        },
    ]);
  }, [initialData, calculations.calculatedCash, expensesByCash]); // Dépendances exactes

  // --- ACTIONS ---
  const handleSaveClosure = async () => {
    setIsSubmitting(true); setError(null);
    try {
      if (!caissePrincipaleData.every(r => r.managerConfirmed)) throw new Error("Veuillez valider toutes les lignes (Manager).");
      
      const unJustified = initialData.sales.filter(s => (s.price_subtotal_incl || 0) <= 0 && !negativeSaleJustifications.map(j => j.sale_id).includes(s.id));
      if (unJustified.length > 0) throw new Error(`Il reste ${unJustified.length} ventes négatives non justifiées.`);

      // ⚠️ LOGIQUE DE SAUVEGARDE SUPABASE ORIGINALE À INSERER ICI
      toast.success("Clôture sauvegardée avec succès !");
    } catch (e: any) {
      setError(e.message);
    } finally {
      ...
      {isReadOnly && existingClosure && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
          <div className="flex gap-2 text-blue-800">
            <Badge variant="outline" className="bg-blue-100">📋 Mode consultation</Badge>
            <span className="font-medium">Clôture du {format(new Date(existingClosure.opening_date), 'dd/MM/yyyy')} - {existingClosure.closure_status}</span>
          </div>
          <Button asChild variant="outline" size="sm"><Link href={`/cloture-vente/${existingClosure.id}`}>📊 Voir détail</Link></Button>
        </div>
      )}

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        
        {/* COLONNE GAUCHE (Billeterie & Formulaires) */}
        <div className="xl:col-span-1 space-y-6">
          <BilleterieCard 
            denominations={denominations} 
            increment={incrementDenomination} 
            decrement={decrementDenomination} 
            totals={{usd: calculations.physicalCashUSD, cdf: calculations.physicalCashCDF, calculated: calculations.calculatedCash}} 
          />

          {/* Finalisation Card */}
          <Card>
            <CardHeader className="pb-4">
              <CardTitle className="flex items-center gap-2 text-lg"><Save className="w-5 h-5" /> Finalisation</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {error && <div className="p-3 bg-red-100 border-red-300 text-red-700 text-sm rounded-lg">{error}</div>}
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-2">Observations</label>
                <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Anomalies..." rows={4} />
              </div>
              <Button onClick={handleSaveClosure} disabled={isReadOnly || isSubmitting} className="w-full bg-green-600 hover:bg-green-700" size="lg">
                <Save className="w-4 h-4 mr-2" /> {isSubmitting ? 'Sauvegarde...' : 'Finaliser la Clôture'}
              </Button>
            </CardContent>
          </Card>
        </div>

        {/* COLONNE DROITE (Données et Tableaux) */}
        <div className="xl:col-span-2 space-y-6">
          <MainCashTable 
            data={caissePrincipaleData}
            onToggleManager={(idx: number) => setCaissePrincipaleData(p => p.map((r, i) => i === idx ? {...r, managerConfirmed: !r.managerConfirmed} : r))}
            onToggleFinancier={(idx: number) => setCaissePrincipaleData(p => p.map((r, i) => i === idx ? {...r, financierConfirmed: !r.financierConfirmed} : r))}
            onValidateAll={(type: 'manager'|'financier') => setCaissePrincipaleData(p => p.map(r => ({...r, [type === 'manager' ? 'managerConfirmed' : 'financierConfirmed']: true})))}
          />

          {/* ⚠️ Ton Composant Secondaire viendra ici (SecondaryCashTable) */}
        </div>
      </div>
    </div>
  );
